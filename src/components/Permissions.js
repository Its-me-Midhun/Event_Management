...
const Permissions = ({ socket }) => {
  ...
  const { permissions, designa, designations, designationSelected } =
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'black' }}>
      <div style={{ width: '15%', margin: '0% 0% 0% 12.8%' }}>
        <select
        ...
        >
          ...
          {designations?.data?.data?.map((e, index) => {
            return (
              <>
      ...
      {User
        ? menus.map((datas, index) => {
            return (
              <div
                className="container"
                style={{ margin: '0 auto', padding: '1%', width: '70%' }}
                key={index}
              >
                <Accordion
                  style={{
                    ...
                    background:
                      'linear-gradient(to right, #ff8008, #ffc837)' /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */,
                    margin: '0% 0% 0% 0%',
                  }}
                >
                  <AccordionSummary
                      ...
                      >
                        <label className="toggleButton">
                          <input
                            ...
                            // checked={Designation.filter((e) => e.Menu === datas)
                            //   .map((e) => e._id)
                            //   .every((elem) => permissions?.includes(elem))}
